...
def _log(level, component, event, **kw):
    """Structured JSON log line — picked up by Cloud Run / Fly.io log drains."""
    print(_json.dumps({
...
def paystack_webhook():
    ...
    if event in REVERSAL_EVENTS:
        _rev_ref = payload.get("data", {}).get("reference") or                    payload.get("data", {}).get("transaction_reference", "")
        if _rev_ref:
            try:
...
def expire_stale_payments():
    ...
    Trigger via Cloud Scheduler or cron: POST /api/admin/expire-payments
    Header: X-Admin-Key: <ADMIN_SECRET>
    """
    # ── Admin auth ───────────────────────────────────────────────────────
    _admin_secret = os.environ.get("ADMIN_SECRET", "")
    if not _admin_secret or request.headers.get("X-Admin-Key") != _admin_secret:
        log_warn("admin", "unauthorized_access", path=request.path,
    ...
    try:
        ...
        conn.commit()
        print(f"[EXPIRY] expired {expired} stale payment(s)")
...
def retry_pending_certifications():
    """
    Retry certification for paid payments that have cert_retry=True.
    Trigger via Cloud Scheduler or cron: POST /api/admin/retry-certifications
    Header: X-Admin-Key: <ADMIN_SECRET>
    """
    # ── Admin auth ───────────────────────────────────────────────────────
    _admin_secret = os.environ.get("ADMIN_SECRET", "")
    ...
    for row in rows:
        try:
            result = trigger_certification(row, row.get("metadata") or {})
            if result:
                retried += 1
                # Clear the retry flag on success
            ...
            print(f"[RETRY] failed for payment {row['id']}: {e}")

    print(f"[RETRY] retried {retried}/{len(rows)} certifications")
