...
# Find and replace
import re
pattern = r'@app\.get\("/api/qrcode-rich/<submission_id>"\)\s+def generate_rich_qr_code.*?(?=@app\.get|if __name__)'
new_content = re.sub(pattern, rich_fix, content, flags=re.DOTALL)

with open('tier4_main.py', 'w') as f:
    f.write(new_content)

print("✅ Fixed rich QR endpoint")
